...
export class AgregarInOutClass extends Component {
    ...
    handleChange = (event) => {
        ...
        if (name === 'fecha') {
            ...
            if (name === 'destino' || name === 'origen' || name === 'carga' || name === 'peso_total') {

                if (name === 'carga' && value.length <= 60) {
                    this.setState({ [name]: value });
                }
                if (name === 'origen' && value.length <= 60) {
                    this.setState({ [name]: value });
                }
                if (name === 'destino' && value.length <= 60) {
                    this.setState({ [name]: value });
                }
                if (name === 'peso_total' && value <= 75000) {
                    this.setState({ [name]: value });
                }
            }
            else {
    ...
    verificarCampos = () => {
        ...
        if (this.state.destino === '') {
            enviar = false;
            this.MensajeError("Agregue el Destino");
        }
        if (this.state.carga === '') {
            enviar = false;
            this.MensajeError("Agregue una descripcion de la Carga");

        }
        if (this.state.peso_total < 100) {
            enviar = false;
            this.MensajeError("Agregue el peso bruto del transporte");
        }
        return enviar
    }
        ...
        return (
            <>
                <div className='container bottom'>
                    <div className='row'>
                        <div className='col'>
                            {this.props.registro.extra === true ? <h3>Modificar Registro de Acceso</h3> : <h3>Registrar Movimiento</h3>};
                        </div>
                    </div>
